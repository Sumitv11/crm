...
import {
  useDeleteUnitMeasurement,
  useGetUnitMeasurements,
  useUnitMeasurementRegister,
  useUpdateUnitMeasurement,
} from "./hook";
import CustomModal from "@/components/Modal/DialogModal";
import Form from "./form";

const UnitofMeasurementsRoute = () => {
  ...
  const {
    ...
    isError,
    refetch,
  } = useGetUnitMeasurements(); // Adding refetch to manually trigger data reload
  const addRegisterMutation = useUnitMeasurementRegister();
  const UpdateServiceMutation = useUpdateUnitMeasurement(serviceId);
  const DeleteServiceMutation = useDeleteUnitMeasurement(serviceId);
  ...
  return (
    <div>
      <h1 className="heading-primary">Unit Of Measurements</h1>
      <div className="p-6 ">
        {" "}
        <DataTable
          rowData={serviceData}
      ...
      <CustomModal
        ...
        dialogTitle={serviceId ? "Edit UoM" : "Create UoM"}
        dialogDescription={
          serviceId ? "Edit UoM here..." : "Create UoM here..."
        }
        formId="service-form"
        width="max-w-4xl"
        height="h-auto"
...
export default UnitofMeasurementsRoute;
